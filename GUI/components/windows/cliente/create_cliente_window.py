from tkinter import Entry, Frame, IntVar, Label, messagebox
...
class CreateClienteWindow(NewWindow):


    def __init__(self, master, int_var:IntVar):
        super().__init__(master, "Agregar CLiente")
        ...
        #self.entry_nombre.insert(0, "adbcd")
    ...
    def save_data(self):
        ...
        try:
            ...
            try:
                ...
                cliente = Cliente(None, nombre, email, tel, direccion)
                
                ClienteRepo().create(cliente)
                messagebox.showinfo(message=f"Cliente:{nombre} creado exitosamente", title="Cliente Creado")
                self.intvar.set(1)
                self.destroy()
